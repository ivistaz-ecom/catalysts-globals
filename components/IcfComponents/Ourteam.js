    ...
    <>
      <div className="bg-[#4F3475] text-white py-16" id="Ourteam">
        <div className="container mx-auto max-w-6xl">
          <h2 className="text-center text-[36px] ">Board of Directors</h2>
        ...
        {modalOpen && (
          <div
            className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50 p-5 overflow-y-auto"
            onClick={closeModal}
          >
            <div
              className="bg-white text-black p-8 rounded-lg lg:w-1/2 relative h-auto max-h-[95%] overflow-y-auto"
              onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside the modal
            >
              {/* Close Button for Mobile */}
              <button
                className="fixed top-10 right-6 p-1 mt-3 text-2xl font-bold text-gray-700 hover:text-gray-900 z-50 block lg:hidden"
                onClick={closeModal}
              >
                &times;
              </button>
              ...
              <button
                className="absolute top-4 right-4 lg:top-6 lg:right-6 p-1 text-2xl font-bold text-gray-700 hover:text-gray-900 z-50 hidden lg:block"
                onClick={closeModal}
              >
                &times;
              </button>
